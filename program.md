...
1. **完善数据同步**
   - 实现逐小时天气数据同步 (`PUT_HOURLYDATA`)
   - 实现天气预警数据同步 (`PUT_WARNDATA`)
   - 同步开关设置现在生效
...
|---------|------|
| `time_svg()` | 时钟图标 - 构建时间 |
| `calendar_svg()` | 日历图标 - 同步天数、到期时间 |
| `user_svg()` | 用户图标 - 构建用户 |
| `branch_svg()` | 分支图标 - Git分支 |
| `hash_svg()` | 井号图标 - Git hash、计费模式 |
| `hourly_sync_svg()` | 沙漏图标 - 逐小时同步开关 |
| `alerts_svg()` | 云+感叹号 - 预警同步开关 |
| `more_link_svg()` | 外链箭头 - 可点击卡片 |
| `help_svg()` | 书本图标 - 帮助文档 |
| `qq_group_svg()` | @符号图标 - QQ群 |
| `afd_svg()` | 爱心图标 - 升级付费、剩余金额 |
| `list_svg()` | 列表图标 - 结果数量 |
| `search_settings_svg()` | 搜索+齿轮 - 搜索范围 |
| `city_svg()` | 城市建筑 - 城市管理Tab |
| `refresh_auth_svg()` | 刷新循环 - 刷新授权 |
| `search_svg()` | 放大镜 - 搜索按钮 |
